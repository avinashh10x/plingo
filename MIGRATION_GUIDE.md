...
**Note**: When `GOOGLE_AI_API_KEY` is set, the AI agent will use Google AI directly. If not set, it falls back to `LOVABLE_API_KEY` (Lovable Cloud only).
...
- Test with: `curl -X POST https://YOUR_PROJECT_ID.supabase.co/functions/v1/generate-content -H "Content-Type: application/json" -d '{"prompt":"test"}'`
...
## 🤖 AI Agent Migration (Detailed)

The AI agent (Buggy) uses the `generate-content` edge function. It supports **two modes**:

### Mode 1: Google AI Direct (Recommended for Self-Hosting)

When you set `GOOGLE_AI_API_KEY`, the agent calls Google's Generative AI API directly.
...
**Model Support:**
| UI Selection | Google Model Used |
|--------------|-------------------|
| gemini-2.0-flash | gemini-2.0-flash |
| gemini-1.5-flash | gemini-1.5-flash |
| gemini-1.5-pro | gemini-1.5-pro |

### Mode 2: Lovable AI Gateway (Lovable Cloud Only)

If `GOOGLE_AI_API_KEY` is not set, the agent uses `LOVABLE_API_KEY` (auto-configured in Lovable Cloud).

**This mode only works on Lovable Cloud** — you must use Mode 1 for self-hosting.

### How the Agent Decides Which Mode to Use

```
GOOGLE_AI_API_KEY exists?
    ├── YES → Use Google AI directly
    └── NO  → LOVABLE_API_KEY exists?
                ├── YES → Use Lovable AI Gateway
                └── NO  → Return error "AI service not configured"
```
...
```json
{
  ...
  "count": 3,
  "tone": "professional",
  "userIdentity": "I'm a tech founder...",
  "guidelines": "Always mention our product name"
}
```
...
|-------|------|---------|-------------|
| `prompt` | string | required | The content request |
| `model` | string | `gemini-2.0-flash` | AI model to use |
| `type` | string | `tweet` | `tweet`, `thread`, `rephrase`, `hashtags`, `chat` |
| `count` | number | `1` | Number of variations (max 10) |
| `tone` | string | `professional` | `professional`, `casual`, `friendly`, `witty`, `formal`, `inspirational` |
| `userIdentity` | string | null | Context about the user |
| `guidelines` | string | null | Additional writing guidelines |
