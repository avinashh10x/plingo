...
const ContentStudio = lazy(() => import("./pages/dashboard/ContentStudio").then(m => ({ default: m.ContentStudio })));
const CalendarPage = lazy(() => import("./pages/dashboard/CalendarPage").then(m => ({ default: m.CalendarPage })));
const DraftsPage = lazy(() => import("./pages/dashboard/DraftsPage").then(m => ({ default: m.DraftsPage })));
...
const ProtectedRoute = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const hasSeenOnboarding = localStorage.getItem('plingo_onboarding_complete');
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <h1 className="text-2xl font-bold text-foreground">Thanks for joining the waitlist!</h1>
          <p className="text-muted-foreground">
            We're currently reviewing your account. You'll receive an email once approved.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      <ConnectPlatformModal open={showConnectModal} onOpenChange={setShowConnectModal} />
    </>
  );
};
      ...
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/dashboard" element={<ProtectedRoute><DashboardLayout /></ProtectedRoute>}>
            ...
            <Route path="studio" element={<Suspense fallback={<PageLoader />}><ContentStudio /></Suspense>} />
            <Route path="calendar" element={<Suspense fallback={<PageLoader />}><CalendarPage /></Suspense>} />
            <Route path="drafts" element={<Suspense fallback={<PageLoader />}><DraftsPage /></Suspense>} />
            <Route path="accounts" element={<Suspense fallback={<PageLoader />}><AccountsPage /></Suspense>} />
            <Route path="settings" element={<Suspense fallback={<PageLoader />}><SettingsPage /></Suspense>} />
            <Route path="create" element={<Navigate to="/dashboard/studio" replace />} />
          </Route>
