import { Clock, Send, Wand2, Plus, Calendar, Link2, FileText, CheckCircle } from 'lucide-react';
...
export const DashboardHome = () => {
  ...
  const connectedPlatformsCount = ['twitter', 'linkedin', 'instagram'].filter(p => isConnected(p as any)).length;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          You have {scheduledPosts.length} posts scheduled. Here's your overview.
        </p>
      </div>
          ...
          <div className="space-y-3">
            {upcomingPosts.length === 0 ? (
              ...
              upcomingPosts.map((post) => (
                <Card key={post.id} className="bg-card border-border hover:border-primary/30 transition-colors cursor-pointer">
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-foreground line-clamp-2">{htmlToPlainText(post.content)}</p>
