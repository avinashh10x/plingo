...
function emitPostsSync(payload: PostsSyncPayload) {
  ...
  window.dispatchEvent(new CustomEvent<PostsSyncPayload>(POSTS_SYNC_EVENT, { detail: payload }));
}
...
export function usePosts() {
  ...
  useEffect(() => {
      ...
      setPosts((prev) => {
        switch (payload.type) {
          case 'upsert': {
            ...
            return exists ? prev.map((p) => (p.id === payload.post.id ? payload.post : p)) : [...prev, payload.post];
          }
          case 'update':
            return prev.map((p) => (p.id === payload.id ? { ...p, ...payload.updates } : p));
          case 'delete':
    ...
    return () => window.removeEventListener(POSTS_SYNC_EVENT, handler as EventListener);
  }, [fetchPosts]);

  const createPost = async (content: string = '', platforms: PlatformType[] = []): Promise<Post | null> => {
    if (!user) return null;

    try {
      const maxOrder = posts.length > 0 ? Math.max(...posts.map(p => p.order_index)) : 0;
  ...
  const schedulePost = async (id: string, scheduledAt: Date, platforms?: PlatformType[]): Promise<boolean> => {
    ...
    try {
      ...
      if (error) {
        ...
        const msg = typeof details === 'string' ? details : (details?.error ?? error.message);
        throw new Error(msg);
      }
      ...
      emitPostsSync({
        type: 'update',
        id,
        updates: { status: 'scheduled' as PostStatus, scheduled_at: nextIso, ...(platforms ? { platforms } : {}) },
      });
  ...
  const bulkSchedule = async (postIds: string[], rule: ScheduleRule, platforms?: PlatformType[]): Promise<boolean> => {
    try {
      ...
      const successCount = data?.scheduled?.filter((s: { success: boolean }) => s.success).length || 0;
  ...
  const publishNow = async (id: string, platform: PlatformType): Promise<boolean> => {
    ...
    try {
      ...
      setPosts(prev => prev.map(p => 
        p.id === id ? { ...p, status: 'posted' as PostStatus, posted_at: new Date().toISOString() } : p
      ));
