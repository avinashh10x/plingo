...
serve(async (req) => {
  ...
  try {
    ...
    const now = new Date();
    let delaySeconds = Math.max(0, Math.floor((scheduledDate.getTime() - now.getTime()) / 1000));
    ...
    const windowStart = new Date(scheduledDate.getTime() - 5 * 60 * 1000).toISOString();
    const windowEnd = new Date(scheduledDate.getTime() + 5 * 60 * 1000).toISOString();
    ...
    if (conflictingSchedules && conflictingSchedules.length > 0) {
      ...
      console.log(`Smart scheduling: Added ${staggerOffset + randomDelay}s delay due to ${conflictingSchedules.length} conflicting schedules`);
    }
    ...
    const attempts: Array<{
      platform: string;
      stage: 'create_schedule' | 'qstash_config' | 'qstash_publish' | 'qstash_exception';
      schedule_id?: string;
      status?: number;
      response?: unknown;
      error?: string;
    ...
    for (const platform of targetPlatforms) {
      ...
      try {
        ...
        console.log('QStash response:', { status: qstashResponse.status, data: qstashData });
    ...
    if (scheduleResults.length === 0) {
      return new Response(JSON.stringify({ error: 'Failed to register schedule with scheduler', attempts }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
