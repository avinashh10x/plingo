...
serve(async (req) => {
  ...
  if (error) {
    ...
    return Response.redirect(
      `${redirectBaseUrl}/dashboard?error=${encodeURIComponent(error)}`
    );
  }
  ...
  if (!platform || !code || !state) {
    ...
    return Response.redirect(
      `${redirectBaseUrl}/dashboard?error=missing_params`
    );
  }
  ...
  try {
    ...
    const stateOrigin =
      typeof stateData?.app_origin === "string" ? stateData.app_origin : null;
    ...
    switch (platform) {
      case "twitter": {
        ...
        console.log(
          "Twitter token exchange completed, status:",
          tokenResponse.ok ? "success" : "failed"
        );
      ...
      case "threads": {
        ...
        const redirectUri = `${supabaseUrl}/functions/v1/oauth-callback?platform=threads`;
        ...
        if (longLivedTokens.error) {
          throw new Error(
            longLivedTokens.error_description || longLivedTokens.error
          );
        }

        // Get user info
        const userResponse = await fetch(
          `https://graph.threads.net/v1.0/me?fields=id,username&access_token=${longLivedTokens.access_token}`
        );
        const userData = await userResponse.json();

        tokenData = {
          access_token: longLivedTokens.access_token,
          expires_in: longLivedTokens.expires_in, // 60 days = 5184000 seconds
          account_id: userData.id || "unknown",
    ...
    const refreshTokenHash = tokenData.refresh_token
      ? hashToken(tokenData.refresh_token)
      : null;
    ...
    return Response.redirect(
      `${redirectBaseUrl}/dashboard?connected=${platform}`
    );
  } catch (error: unknown) {
    console.error("OAuth callback error:", error);
    const message = error instanceof Error ? error.message : "Unknown error";
    return Response.redirect(
      `${redirectBaseUrl}/dashboard?error=${encodeURIComponent(message)}`
    );
  }
});
