...
serve(async (req) => {
  ...
  try {
    ...
    // Verify QStash signature if present (for scheduled posts via QStash)
    ...
    if (signatureJwt) {
      ...
      let jwtPayload: { sub?: string; body?: string; iss?: string } | null =
        null;
      ...
      console.log("Direct user call detected, verifying Supabase auth...");
      ...
      try {
        ...
        if (authError || !user) {
          console.error("Invalid Supabase auth token:", authError);
          return new Response(
        ...
        return new Response(
          JSON.stringify({ error: "Unauthorized - auth verification failed" }),
          {
            status: 401,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          }
    ...
    if (
      ...
      new Date(platformData.expires_at) < new Date()
    ) {
